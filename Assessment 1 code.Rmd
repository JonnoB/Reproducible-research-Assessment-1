...
###Load libraries
the libraries and packages required for this analysis are loaded below, it is assuemd that the user has installed the relevant packages
...
```{r}
packages <- c("ggplot2","reshape2","RCurl", "plyr")
...
```{r, eval=FALSE}
#This code chunk checks the raw data folder to see if the file is already present if not it downloads it from the coursera website and unzips it ready for use. this code chunk needs to be turned off when knitting other wise the script won't work. Need to find a solution where Knitr can download and open a zip file
...
The total number of rows with missing values is `r incom` which is `r incom/nrow(data)`/% of the total data set.
...
ggplot(interval.week, aes(x=Interval, y=Steps)) + geom_line() + facet_grid(Week~.)+geom_smooth()+ggtitle("Line plot showing aggregated number of steps per time interval facet by week/weekend")+xlab("time interval") + ylab("average number of steps")
```
